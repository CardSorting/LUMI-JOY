...
> **Generated At**: 2026-08-16T07:34:55.555Z
...
| Mean heterogeneous case latency | 67.41 ms | observed |
| Workload throughput | 14.83 cases/sec | observed |
...
| architecture | Core abstract contracts remain connected | 0.14 ms | PASS | agent, session, hands, ears, and tool-registry contracts verified |
| execution | Local frame commits an explicit successful outcome | 0.93 ms | PASS | frame 1 completed in 0.6 ms |
| state | Snapshot rewind restores frame and message state | 0.31 ms | PASS | restored frame 1 with 2 messages |
| governance | Completion gate fails closed without evaluated evidence | 0.48 ms | PASS | 5 fail-closed states rejected; 2/2 evaluated required criteria accepted |
| safety | Modern command safety and diagnostics are active | 0.29 ms | PASS | interactive editor blocking and port-collision guidance verified |
...
| health | Subsystem health aggregation remains optimal | 0.08 ms | PASS | 3/3 registered subsystems healthy |
...
| Performance SLA: Execution Throughput | 8566.85 frames/sec | >= 1000 frames/sec | PASS |
| Performance SLA: State Rewind Latency | 0.019 ms p95 | < 0.1 ms p95 | PASS |
