...
export interface InputOptions {
	/** Replaces entered graphemes while rendering, for credentials and other secrets. */
...
export class Input implements Component, Focusable {
	...
	render(width: number): string[] {
		...
		const displayValue = this.maskCharacter
			? Array.from(segmenter.segment(this.value), () => this.maskCharacter!).join("")
			: this.value;
		const displayCursor = this.maskCharacter
			? Array.from(segmenter.segment(this.value.slice(0, this.cursor)), () => this.maskCharacter!).join("").length
			: this.cursor;
