...
> 📌 **Current Handoff Status**: **Passes 1–8 Complete**. The active next planned evolutionary pass is **Pass 9: Workspace Mention Resolution (`packages/codemarie/src/core/mentions`)**.

---

## 3. Playbook: How to Execute Next Evolution Pass (e.g., Pass 9)
...
5. **Verification**: Run `npm run check` and `npm start` (`npx tsx src/index.ts`). Log ADR in `.wiki/adr/`.
