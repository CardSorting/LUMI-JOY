...
```
  [DONE] Pass 1 ──► [DONE] Pass 2 ──► [DONE] Pass 3 ──► [DONE] Pass 4 ──► [DONE] Pass 5 ──► [DONE] AKD-DSO
                                                                                               │
  [PLANNED] Pass 14 ◄── [PLANNED] Pass 13 ◄── [PLANNED] Pass 12 ◄── [PLANNED] Pass 11 ◄── [PLANNED] Pass 10 ◄── [ACTIVE NEXT] Pass 9 ◄── [DONE] Pass 8 ◄── [DONE] Pass 7 ◄── [DONE] Pass 6 ◄──┘
```
...
| **Pass 9** | `[ACTIVE NEXT]` | `packages/codemarie/src/core/mentions` | Context `@mention` resolution (`@file`, `@symbol`, `@git`, `@terminal`) in `PromptComposer` | [Pass 9 Blueprint](#pass-9-workspace-mention-resolution) |
| **Pass 10** | `[PLANNED]` | `packages/codemarie/src/core/policy` | Zombie symbol detection & dependency analysis in `ModuleDecomposer` | [Pass 10 Blueprint](#pass-10-zombie-symbol--module-decomposition) |
...
- **Files Modified/Created**:
  ...
  - [src/sessions/extensions/arena-allocator.ts](file:///Users/bozoegg/Desktop/LUMI-NEW/src/sessions/extensions/arena-allocator.ts): Implement `ArenaAllocator` slab allocator.
  - [src/sessions/extensions/session-store.ts](file:///Users/bozoegg/Desktop/LUMI-NEW/src/sessions/extensions/session-store.ts): Integrate pre-allocated turn buffer allocation.
...
- **Files Modified/Created**:
  ...
  - [src/tooling/extensions/ears.ts](file:///Users/bozoegg/Desktop/LUMI-NEW/src/tooling/extensions/ears.ts): Add `TerminalProgressRenderer` and `ProtocolEars.emitProgress()` streaming.
...
- **Files to Modify/Create**:
  - `src/agents/extensions/mention-resolver.ts`: Create `MentionResolver` parser & resolver.
  - [src/agents/extensions/prompt-composer.ts](file:///Users/bozoegg/Desktop/LUMI-NEW/src/agents/extensions/prompt-composer.ts): Integrate `MentionResolver` into prompt compilation loop.
