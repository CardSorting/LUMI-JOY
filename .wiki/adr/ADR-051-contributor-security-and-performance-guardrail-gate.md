...
## Baseline Values (as of 2026-08-16T07:05:11.963Z)
- Composition Manifest: **377/377** required components in OPTIMAL cohesion.
- Fast-Path Mean Turn Tick Latency: **$0.15\text{ ms}$** (SLA: $< 1.0\text{ ms}$).
- Local Frame Throughput: **$6749.16\text{ frames/second}$** (SLA: $\ge 1,000\text{ frames/second}$).
- State Snapshot Rewind Latency: **$0.018\text{ ms p95}$** (SLA: $< 0.1\text{ ms p95}$).
